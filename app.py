...
# 2. CARREGAMENTO DINÂMICO COM VERIFICAÇÃO
@st.cache_resource
def load_assets():
    # Caminho absoluto para a raiz do repositório no Streamlit Cloud
    ...
    # Validação amigável para o desenvolvedor
    if not os.path.exists(db_path):
        st.error(f"Arquivo de banco de dados não encontrado no caminho: {db_path}")
        st.info("Verifique se a pasta 'data/database/' e o arquivo 'pokemon_data.db' estão no seu GitHub.")
        st.stop()
...
# Tenta carregar e define globais
...
try:
    ...
    if st.button("PREVER VENCEDOR"):
        ...
        st.plotly_chart(criar_grafico_comparativo(p1_d, p2_d, p1_n, p2_n))
